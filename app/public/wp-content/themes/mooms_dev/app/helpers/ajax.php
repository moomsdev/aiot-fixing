...
function sendContactForm() {
    ...
    // Sau khi gửi cho quản trị, thử gửi email xác nhận tới khách hàng (không ảnh hưởng kết quả chính)
    $user_subject = sprintf('%s: Thank you for contacting us', $blogName);
    $user_html = sprintf(
        '<p>Dear %s %s,</p><p>Thank you for contacting %s. We have received your message with the following details:</p><ul><li>Name: %s %s</li><li>Email: %s</li><li>Phone: %s</li></ul><p>Message:</p><p>%s</p><p>We will get back to you soon.</p><p>Best regards,<br>%s</p>',
        esc_html($first_name),
        esc_html($last_name),
        esc_html($blogName),
        esc_html($first_name),
    ...
    $user_headers = [
        'Content-Type: text/html; charset=UTF-8',
        'Reply-To: ' . esc_html($blogName) . ' <' . sanitize_email(get_option('admin_email')) . '>',
    ];
    if (!empty($email) && is_email($email)) {
        $user_mail_sent = wp_mail($email, $user_subject, $user_html, $user_headers);
        if (!$user_mail_sent) {
            error_log('Confirmation email failed to send to user: ' . $email);
        }
    }
